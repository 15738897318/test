...
- (NSArray*) detectFrontalFaces:(cv::Mat) frame
{
    ...
    NSMutableData *frame_in = [self NsDataFromCvMat:&frame_gray];
...
- (int) assessFaces:(NSArray *)faces withLowerBound:(cv::Rect)ROI_lower
{
	...
	for(int i = 0; i < faces.count; i++) {
        CIImage *ciImage = [faces objectAtIndex:i];
        
		if ((ciImage.extent.size.width >= ROI_lower.width) && (ciImage.extent.size.height >= ROI_lower.height))
		{
			return 1;
		}
	}
