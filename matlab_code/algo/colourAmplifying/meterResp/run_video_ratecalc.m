file_template = 'baby-iir-from-0.05-to-0.4-alpha-10-level-16-chromAtn-0.1.mp4';

% file_template = 'baby-ideal-from-0.05-to-0.5-alpha-150-level-6-chromAtn-0.avi';

file_template = 'baby2-ideal-from-2.3333-to-2.6667-alpha-150-level-6-chromAtn-1.mp4';
...
hr_array = {};
k = 0;
if ~exist(fullfile(src_folder, results_file), 'file')
	fileID = fopen(fullfile(src_folder, results_file), 'a');
...
for file_ind = 1 : length(file_list)
	...
	for channel_ind = 1 : length(channels_to_process)
		...
		hr_output = func_rate_calc(vidFileName, ...
									window_size_in_sec, overlap_ratio, ...
									max_bpm, cutoff_freq, ...
									colour_channel, ref_reading, ...
