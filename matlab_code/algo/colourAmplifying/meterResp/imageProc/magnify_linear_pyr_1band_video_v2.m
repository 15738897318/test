...
function magnify_linear_pyr_video(vidFile, outDir, ...
    ...
    outName = fullfile(outDir,[vidName '-' pyramid_style 
    ...
    filtered_pyramid = ideal_bandpassing(pyramid, 1, freq_band_low_end, freq_band_high_end, samplingRate);
    disp('Finished')
    
    %% amplify
    filtered_pyramid = permute(filtered_pyramid, [1, 4, 2, 3]);
    for i = 1 : size(filtered_pyramid, 3)
    	for j = 1 : size(filtered_pyramid, 4)
    		temp_array = squeeze(filtered_pyramid(:, :, i, j));
    		
    		temp_array = temp_array * C_matrix';
    		
    		filtered_pyramid(:, :, i, j) = temp_array;
    	end
    end
    filtered_pyramid = permute(filtered_pyramid, [1, 3, 4, 2]);
    ...
    for i = startIndex : endIndex
        ...
        if k <= size(filtered_pyramid, 1)
			...
			filtered = squeeze(filtered_pyramid(k, :, :, :));
