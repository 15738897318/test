...
for file_ind = 1 : length(resultFiles)
	...
	% ================= Plot the Bland-Altman for the best settings
	handle_ba(file_ind, 1) = plotBlandAltman(dataset_numbers(:, dataset_errors.acf_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.acf_meanare_min_setting, 3), ...
											 strcat(resultFiles{file_ind}, '-ACF'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-BlAl-ACF-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-BlAl-ACF-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end

	handle_ba(file_ind, 2) = plotBlandAltman(dataset_numbers(:, dataset_errors.pda_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.pda_meanare_min_setting, 4), ...
											 strcat(resultFiles{file_ind}, '-PDA'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-BlAl-PDA-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-BlAl-PDA-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end

	handle_ba(file_ind, 3) = plotBlandAltman(dataset_numbers(:, dataset_errors.avg_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.avg_meanare_min_setting, 5), ...
											 strcat(resultFiles{file_ind}, '-Avg'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-BlAl-Avg-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-BlAl-Avg-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end

	% ================= Plot the regression plots
	handle_reg(file_ind, 1) = plotRegression(dataset_numbers(:, dataset_errors.acf_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.acf_meanare_min_setting, 3), ...
											 strcat(resultFiles{file_ind}, '-ACF'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-Reg-ACF-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-Reg-ACF-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end

	handle_reg(file_ind, 2) = plotRegression(dataset_numbers(:, dataset_errors.pda_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.pda_meanare_min_setting, 4), ...
											 strcat(resultFiles{file_ind}, '-PDA'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-Reg-PDA-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-Reg-PDA-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end

	handle_reg(file_ind, 3) = plotRegression(dataset_numbers(:, dataset_errors.avg_meanare_min_setting, 1), ...
											 dataset_numbers(:, dataset_errors.avg_meanare_min_setting, 5), ...
											 strcat(resultFiles{file_ind}, '-Avg'));
	if exportFlag
		temp = strsplit(resultFiles{file_ind}, '.');
 		exportThisPlot('name', [temp{1} '-Reg-Avg-Best'], 'plotPath', fullfile(resultFolder, 'graphs'))
 		exportThisPlot('name', [temp{1} '-Reg-Avg-Best'], 'plotPath', fullfile(resultFolder, 'graphs'), 'plotType', 'fig')
 	end
